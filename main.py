...
def create_serial_num(local_db: str = 'serial_list'):
    """
    ...
    if get_db_info(db=local_db) == 0:
        newHU = serialNum(date[0], date[1], date[2], model='0311', count=str('1'))
        print(repr_message(str(newHU)))
        return confirm(save_to_db, newHU, db=local_db)
    else:
        newHU = serialNum(date[0], date[1], date[2], model='0311', count=str(get_count_from_db(db=local_db) + 1))
        print(repr_message(str(newHU)))
        return confirm(save_to_db, newHU, db=local_db)
...
if __name__ == '__main__':
    ...
    # scanQR ='2022042603110001'
    # scanQR = get_newest_value(db='serial_list')
